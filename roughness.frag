...
vec3 sample_pixel(vec2 uv, int sample_index) {
         ...
         ((sphere_intersection.distance < 0.0 ||
          ...
          (ground_plane_intersection.distance < 0.0 ||
           ground_plane_intersection.distance >
               reflection_plane_intersection.distance))) {
    ray = Ray(
        reflection_plane_intersection.position,
        normalize(reflect(ray.direction, reflection_plane_intersection.normal) +
                  random(reflection_plane_intersection.position +
                         float(iFrame) + float(sample_index * 1000)) *
                      0.025));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  for (int i = 0; i < 10; i++) {
    // For antialiasing:
    vec2 pixel_offset = vec2(random(fragCoord + float(i)) - 0.5,
                             random(fragCoord + float(i * 1000)) - 0.5);
    color += sample_pixel(uv + pixel_offset / iResolution.xy, i) / 10.0;
  }
