...
Ray cameraRay(vec2 uv, vec2 resolution, vec3 origin) {
  ...
  vec2 sensor_coords = (uv - 0.5) * sensor_size;
  vec3 point_on_sensor = vec3(sensor_coords.x, -FOCAL_LENGTH, sensor_coords.y);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  Ray ray =
      cameraRay(uv, iResolution.xy,
                vec3(2 * sin(iTime / 10), 20, 2 * cos(iTime / 10)));
                // vec3(0, 50, 0));
